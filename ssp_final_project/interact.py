...
def generate_response(history, text, tokenizer, model, args, device):
    ...
    system = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS["system"])

    history = history + [person] + [tokenizer.convert_tokens_to_ids(word) for word in text]
    dialogue_ids = [start_context] + history + [end_context] + [start_action] + [end_action] + \
                   [start_know] + [end_know] + [start_response]
    ...
    for _ in range(args.max_len):
        ...
        if next_token == end_response:  # 遇到[END_RESPONSE]则表明response生成结束
            break
        generated.append(next_token.item())
        curr_input_tensor = torch.cat((curr_input_tensor, next_token), dim=0)

    history = history + [system] + generated
    text_1 = tokenizer.convert_ids_to_tokens(generated)
    return text_1, history
...
def main():
    ...
    while True:
        ...
        text_1, history_context = generate_response(history_context, text, tokenizer, model, args, device)
        print(''.join(text_1))
